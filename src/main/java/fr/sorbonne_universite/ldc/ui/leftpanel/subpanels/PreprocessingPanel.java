...
/**
 * Creates the middle panel of the {@link LeftPanel}, containing preprocessing controls
 * (contrast enhancement and median filter).
 *
 * <p>
 * This panel is the first step of the pipeline. It allows the user to :
 * </p>
 * <ul>
 *   <li>Enhance the contrast of the image (visual only, does not modify pixel values).</li>
 *   <li>Apply a median filter to reduce noise while preserving edges, with a live preview.</li>
 *   <li>Restrict the processed stack to a custom slice range.</li>
 * </ul>
 *
 * <p>
 * Heavy operations (median filter preview and apply) are executed asynchronously
 * using {@link SwingWorker} to keep the UI responsive, with cancellation support.
 * </p>
 */
@SuppressWarnings("serial")
public class PreprocessingPanel extends JPanel implements PipelineSubPanel {

    /** The parent {@link LeftPanel} used to access the current image and pipeline state. */
    private LeftPanel leftPanel;

    /** The LDC plugin service, used to read and update pipeline settings. */
    @Parameter
    private LDCService ldc;

    // ── Contrast components ───────────────────────────────────────────────────

    /** Checkbox to enable or disable contrast enhancement. */
    private JCheckBox enhanceCheckbox;

    /** Spinner to set the saturation percentage for contrast enhancement. */
    private JSpinner enhanceSaturatedSpinner;

    /** Button to reset the saturation percentage to its default value. */
    private JButton enhanceSaturatedResetButton;

    // ── Median filter components ──────────────────────────────────────────────

    /** Checkbox to enable or disable the median filter. */
    private JCheckBox medianCheckbox;

    /** Checkbox to toggle the live preview of the median filter on the current slice. */
    private JCheckBox medianPreviewCheckbox;

    /** Button to reset the median filter radius to its default value. */
    private JButton medianRadiusResetButton;

    /** Text field to input the median filter radius in pixels. */
    private JTextField medianRadiusField;

    /** Animated loading indicator shown while a background worker is running. */
    private JLabel loadingLabel;

    /** Button to cancel the currently running background worker. */
    private JButton cancelButton;

    // ── Apply section components ──────────────────────────────────────────────

    /** Text field to specify a custom slice range to process (e.g. {@code "1-3,5,8-10"}). */
    private JTextField applyRangeField;

    /** Button to apply all selected preprocessing parameters to the image stack. */
    ...
    /**
     * Constructs the {@code PreprocessingPanel}, initializing all UI components
     * and wiring their action listeners to the corresponding logic methods.
     *
     * <p>All components are disabled at construction time and will be enabled
     * once an image is loaded in the plugin.</p>
     *
     ...
     * @param blocking  {@code true} if the current thread must wait for the worker to complete.
     *                  This is the case on new parameters import.
     */
     ...
     *
     * @param enable          {@code true} to enable inputs, {@code false} to disable them.
     * @param resetParameters {@code true} to reset all inputs to their default values.
     *                        Taken into account ONLY if {@code enable} is {@code false}.
     */
     ...
     *
     * @param enable {@code true} to enable components, {@code false} to disable them.
     */
    @Override
    ...
    /**
     * Resets all UI components to their default values and re-enables
     * only the top-level checkboxes (contrast and median filter).
     *
     ...
     * restoring the state of the contrast and median filter controls
     * from the {@link LDCService}.
     *
    ...
    /**
     * Applies the current preprocessing parameters to the entire image stack.
     *
     * <p>Executes asynchronously, blocking the calling thread until completion.
     * Used during parameter import to replay the preprocessing step.</p>
     */
    @Override
